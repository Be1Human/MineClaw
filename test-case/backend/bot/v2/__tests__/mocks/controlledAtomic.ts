...
export async function runControlledAtomic(request: ActionRequest, fixture: AtomicFixture) {
      ...
      run:async context=>{
        ...
        if (req.type==='invoke_behavior') {
          ...
          return new BehaviorRunner({bus:fixture.bus,getWorld:()=>fixture.worldState!}).run(behavior,{
            ...context,command:{ref:{id:`behavior:${behavior.id}`,version:'1'},args:req.target?.behaviorParams ?? {}},
          });
        }
  ...
  const intent: OperationIntent={operationId:randomUUID(),owner:{kind:'task',taskId:'atomic-unit',generation:1},
    command:{ref:{id:'fixture:root',version:'1'},args:{}},scope:{dimension:'overworld',targetRefs:[],bindings:[]},
