...
function exportName(pluginId) {
  return `create${pascal(pluginId)}Plugin`;
}
...
export function buildPluginIndex({
  ...
  for (const plugin of plugins) {
    ...
    lines.push(`import { ${exportName(plugin.pluginId)} } from '${modulePath}.js';`);
  }
  lines.push('');
  ...
  for (const plugin of plugins) {
    if (!plugin.entryFile) continue;
    lines.push(`  ['${plugin.entryKey}', ${exportName(plugin.pluginId)}()],`);
  }
  lines.push(']);');
