...
.trace-header h1 { margin:4px 0 3px; color:var(--mc-text); font:700 20px var(--mc-font-body); }.trace-header p { margin:0; color:var(--mc-text-muted); font-size:12px; }
...
.trace-filters { flex:none; display:grid; grid-template-columns:minmax(170px,1fr) 130px 150px 180px auto auto; gap:8px; align-items:end; }.trace-filters label { display:flex; flex-direction:column; gap:4px; min-width:0; }.trace-filters label span { color:var(--mc-text-muted); font-size:10px; }.trace-filters input,.trace-filters select { min-width:0; height:34px; }
...
.event-ledger { position:relative; padding:6px 0; }.event-row { width:100%; min-height:62px; display:grid; grid-template-columns:34px 14px minmax(0,1fr); padding:7px 9px; border:0; background:transparent; box-shadow:none; text-align:left; }.event-row:hover,.event-row.active { background:var(--mc-surface-hover); }.event-row.active { box-shadow:inset 2px 0 var(--mc-accent); }.event-seq { color:var(--mc-text-muted); font:10px var(--mc-font-mono); padding-top:3px; }.event-rail { position:relative; }.event-rail::before { content:''; position:absolute; left:5px; inset-block:0; width:1px; background:var(--mc-border-strong); }.event-rail i { position:absolute; z-index:1; left:2px; top:3px; width:8px; height:8px; border-radius:50%; background:var(--mc-text-muted); border:2px solid var(--mc-bg-elevated); }.event-row.request .event-rail i { background:#62a9e8; }.event-row.success .event-rail i { background:var(--mc-accent); }.event-row.danger .event-rail i { background:var(--mc-danger); }.event-row.warn .event-rail i { background:var(--mc-warning); }.event-copy { min-width:0; display:flex; flex-direction:column; gap:3px; }.event-copy>span { display:flex; justify-content:space-between; gap:6px; }.event-copy em.goalagent { color:#c8a2ff; border-color:rgba(200,162,255,.25); }.event-copy time { color:var(--mc-text-muted); font:9px var(--mc-font-mono); }.event-copy strong { color:var(--mc-text-secondary); font-size:11px; }.event-copy small { overflow:hidden; color:var(--mc-text-muted); font-size:10px; text-overflow:ellipsis; white-space:nowrap; }
